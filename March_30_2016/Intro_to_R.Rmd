---
title: "Introduction to R (with candy!)"
author: "Dan Chitwood, Donald Danforth Plant Science Center (St. Louis, MO)"
date: "March 27, 2016"
...
One of the most powerful aspects of the R community is being able to use the tremendous amount of packages others have written. You can check out the multitudes of packages at the [Comprehensive R Archive Network (CRAN)](https://cran.r-project.org/web/packages/), [Bioconductor](https://www.bioconductor.org/) (a bioinformatics-specific suite of R software), or many developers that make their packages available through [GitHub](https://github.com/trending/r). 

Let's install a package and call it up. My favorite package is [ggplot2](http://docs.ggplot2.org/current/), by [Hadley Wickham](https://twitter.com/hadleywickham). It is a powerful visualization package. You should not be using excel for anything after learning R, but especially for your visualizations, which `ggplot2` makes very aesthetic. We will be going over how to make `ggplot2` graphs over the next few lectures. But for now, let's make sure we have it installed to save time.
...
If you want to learn more about `ggplot2`, browse over its amazing [documentation](http://docs.ggplot2.org/current/). Scroll through all the different types of graphs and if something catches your eye that you think might be useful, check out the examples and make your own graph! 
...
Or, if you would like very aesthetic colours, you should check out the [color brewer](http://colorbrewer2.org/) website and the colour brewer function `scale_fill_brewer()`. Note that colour schemes at colour brewer are for sequential, divergent, and qualitative data types. Also, be sure to check out the `scale_fill_gradient` options at the [ggplot2 website](http://docs.ggplot2.org/current/)!
...
Wouldn't be nice to know the identity of each candy in this plot? And if two candies overlap, wouldn't it be nice to see both names? Let's install the `ggrepel` package and used the function `geom_repel_text`
...
library(ggrepel)
```
And now lets make the graph using `geom_text_repel`: 
